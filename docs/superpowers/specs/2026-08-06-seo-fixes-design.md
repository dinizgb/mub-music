...
| Canonical mismatch    | Audit hits `https://mubmusic.com` (no slash); preferred URL is `https://mubmusic.com/` with `trailingSlash: true`  |
...
- Trailing slash required on all public paths (matches `next.config.js` `trailingSlash: true`).
- Canonical, Open Graph `url`, and sitemap `<loc>` values always use the trailing-slash form.
- Non-trailing-slash requests must redirect to the slash form (verify Next + hosting; do not serve both as 200).
